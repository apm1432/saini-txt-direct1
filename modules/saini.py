...
async def decrypt_and_merge_video(mpd_url, keys_string, output_path, output_name, quality="720"):
    try:
        ...
        if globals.vidwatermark != "/d":
            watermark_filter = f"drawtext=fontfile=vidwater.ttf:text='{globals.vidwatermark}':fontcolor=black@0.7:fontsize=h/13:x=(w-text_w)/4:y=(h-text_h)/9"
           # watermark_filter = f"drawtext=fontfile=vidwater.ttf:text='{globals.vidwatermark}':fontcolor=black@0.7:fontsize=h/12:x=(w-text_w)/5:y=(h-text_h)/6:enable='between(t,0,180)'"
            cmd4 = f'ffmpeg -i "{output_path}/video.mp4" -i "{output_path}/audio.m4a" -vf "{watermark_filter}" -c:v libx264 -preset ultrafast -crf 25 -c:a copy "{output_path}/{output_name}.mp4"'
        else:
